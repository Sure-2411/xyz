...
export class FlightBookingComponent implements OnInit, OnDestroy {
  ...
  confirmBooking(): void {
    ...
    this.paymentService
      .bookFlight(
        ...
        this.flightId,
        formattedDate,
        this.bookingForm.value.passengers // Passengers as quantity
      )
      .subscribe({
        next: (response) => {
          ...
          if (response.sessionUrl) {
            window.location.href = response.sessionUrl; // Redirect to payment
          } else {
        ...
        error: (err) => {
          ...
          this.showConfirmationPopup = false; // Hide popup on error
        },
      });
  }
  ...
  closePopup(): void {
    this.showConfirmationPopup = false; // Ensure the popup is hidden
    this.clearMessages(); // Clear any error or success messages
  }
